...
class RNNCustom2FixPerturb_noise_prob_train(AbstractRNNCell):
    # x should be size (batch, time_points, 3)
    # (:,:,0) is the normal inputs, and (:,:,1:2) is binary representing time when there are noise perturbation on either RNN A or RNN B
...
#        self.output_size = self.units
    ...
    def call(self, inputs, states, training=False):
        ...
        input_0 = inputs[:, 0:1]  # Shape: (batch_size, 1)
        input_A = tf.cast(inputs[:, 1:2], dtype=tf.float32)  # Shape: (batch_size, 1)
        input_B = tf.cast(inputs[:, 2:3], dtype=tf.float32) # Shape: (batch_size, 1)
        ...
        h = backend.dot(input_0, self.kernel)
        ...
        output_A=output_A+ input_A * tf.random.normal(tf.shape(output_A),mean=0,stddev=self.pert_noisesd)# add noise to RNN A
        output_B=output_B+ input_B * tf.random.normal(tf.shape(output_B),mean=0,stddev=self.pert_noisesd)# add noise to RNN B     
