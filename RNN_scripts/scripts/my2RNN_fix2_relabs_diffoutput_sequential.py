...
conProb=[0.0001,0.001,0.01,0.1,0.5]
weight_max=[0.2,0.2,0.2,0.2]
...
def makeInOut(sample_size,trial_num,inputdur,nInput,min_dur,max_dur,dt):
    ...
    y=-0.5*np.ones((sample_size,total_time,4)) # y is the ouput
    ...
    for i in range(sample_size):
        ...
        for j in np.arange(trial_num):
            ...
            if j % 2==binvec[i % 2]:
                ...
                y[i,vec:vecbf,3]=np.linspace(-0.5,-0.5+minratio,num=vecbf-vec) # timing 2
            else:
                vec-=max_dur+random.randint(-int(max_dur*noise_range),int(max_dur*noise_range))
                cont_output=-context_mag
                y[i,vec:vecbf,3]=np.linspace(-0.5+minratio,0.5,num=vecbf-vec) # timing 2
            In_ons[i,int(-j-1)]=vec
            Dur=vecbf-vec
            x[i,vec:vec+inputdur,:]=1
            y[i,vec:vecbf,0]=np.linspace(-0.5,0.5,num=Dur)        # relative timing 1
            y[i,vec:vecbf,1]=np.arange(-0.5,-0.5+(Dur/max_dur)-1e-10,1/max_dur) # aboslute timing 1
            y[i,vec:vecbf,2]=cont_output      # context timing 2
...
def column_corr(A,B):
    ...
    correlation = np.dot(B.T, A)/(A.shape[0]-1)
    return correlation



for loopind in range(len(conProb)):
    save=True
    if save:
        # set name of the folder
        weight_name=get_numbers_after_period(conProb[loopind])
        directory = f"2sameInputs_probvar{conProb[loopind]}_weightmax{weight_max[loopind]}"      
    ...
    trial_num=3
    con_prob=conProb[loopind] # probability of connection between neurons
    maxval=weight_max
    ReLUalpha=0.2
