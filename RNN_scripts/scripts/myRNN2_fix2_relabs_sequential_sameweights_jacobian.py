...
from RNNCustom2SameWeight import RNNCustom2sameweights
from RNNCustom2SameWeight_Jacobian import RNNCustom2sameweights_Jacobian
from QRDCell import QRDcell
from CustomConstraint_withMax_sameweights import IEWeightandLim_same, IEWeightOut
from Winitial_sameweights import OrthoCustom_sameweights
...
weight_max=[0.003,0.005,0.0075,0.01,0.0125,0.015,0.0175,0.025,0.05,2]
weight_max=[0.0001,0.2,0.00001]
connection_prob=[0.005,0.005,0.005]
...
def build_model():
    ...
    hidden1=RNN(RNNCustom2sameweights(nUnit, 
                          ...
                          recurrent_initializer=OrthoCustom_sameweights(gain=0.5, seed=seed1 , nUnit=nUnit, nInh=nInh, conProb=con_prob),
                          recurrent_constraint=IEWeightandLim_same(nInh=nInh,A_mask=A_mask,B_mask=B_mask,maxval=maxval),
...
def column_corr(A,B):
    # returns column wise correlation coefficient 
    # correlation[i,j] is the correlation coefficient of A[:,i] and B[:,j]
    A = (A - A.mean(axis=0))/A.std(axis=0)
    B = (B - B.mean(axis=0))/B.std(axis=0)
    correlation = np.dot(B.T, A)
    return correlation
...
for weight_ind in range(len(weight_max)):
    ...
    if save:
        ...
        directory = f"2sameInputs_prob{connection_prob[weight_ind]}_sameweightmax{weight_name}"   
    ...
    trial_num=3
    con_prob=connection_prob[weight_ind] # probability of connection between neurons
...
for ss in range(2):
    ...
    xticks = axs[ss].get_xticks()
    # Rescale tick labels
    scaled_xticks = np.round(xticks * 0.01)
    # Set new tick labels
    axs[ss].set_xticklabels(scaled_xticks)
...
crosscorr_A=column_corr(act_avg_A.T,act_avg_A_2.T)
crosscorr_B=column_corr(act_avg_B.T,act_avg_B_2.T)
...
fig, axs = plt.subplots(1,2)
axs[0].imshow(crosscorr_A, aspect='auto', cmap=parula,interpolation='none')
axs[0].set_title(f'Cross Correlation A')
axs[1].imshow(crosscorr_B, aspect='auto', cmap=parula,interpolation='none')
axs[1].set_title(f'Cross Correlation B')
if save:
    plt.savefig(os.path.join(savepath,f"cross correlation"),transparent=True,dpi=600)
plt.show


fig, axs = plt.subplots(1,2)
axs[0].imshow(-np.log(1.001-np.minimum(1,crosscorr_A)), aspect='auto', cmap=parula,interpolation='none')
axs[0].set_title(f'Cross Correlation A')
axs[0].set_aspect('equal', 'box')
axs[1].imshow(-np.log(1.001-np.minimum(1,crosscorr_B)), aspect='auto', cmap=parula,interpolation='none')
axs[1].set_title(f'Cross Correlation B')
axs[1].set_aspect('equal', 'box')
if save:
    plt.savefig(os.path.join(savepath,f"cross correlation_log"),transparent=True,dpi=600)
plt.show


# firing field correlation
...
IndM6 = np.argmax(neuavg_3[:600, :], axis=0)
IndM12 = np.argmax(neuavg_3[600:, :], axis=0)
# Filter neuavg based on conditions
mask = (IndM6 > afterrew * 10) & (IndM12 > afterrew * 10)
neuavg_3 = neuavg_3[:, mask]


col_keep=np.logical_and(np.std(neuavg_3[:600, :],axis=0)>0 ,np.std(neuavg_3[600:1800, :],axis=0)>0)
neuavg_3=neuavg_3[:,col_keep]
IndM6 = np.argmax(neuavg_3[:600, :], axis=0)
IndM12 = np.argmax(neuavg_3[600:1800, :], axis=0)

# Calculate differences
diff6s = IndM6[:, np.newaxis] - IndM6[np.newaxis, :]
diff6s = np.minimum(np.minimum(np.abs(diff6s), np.abs(diff6s + 600)), np.abs(diff6s - 600))

diff12s = IndM12[:, np.newaxis] - IndM12[np.newaxis, :]
diff12s = np.minimum(np.minimum(np.abs(diff12s), np.abs(diff12s + 1200)), np.abs(diff12s - 1200))

# Calculate correlation coefficients
#corrcoef6 = np.corrcoef(neuavg_3[:600, :].T)
#corrcoef12 = np.corrcoef(neuavg_3[600:1800, :].T)



corrcoef6=column_corr(neuavg_3[:600, :],neuavg_3[:600, :])
corrcoef12=column_corr(neuavg_3[600:1800, :],neuavg_3[600:1800, :])

# Create upper triangular mask
triu_one = np.triu(np.ones(neuavg_3.shape[1]), 1).astype(bool)

# Extract upper triangular elements
diff6s_upper = diff6s[triu_one]
corrcoef12_upper = corrcoef12[triu_one]

# First figure
plt.figure()
plt.subplot(1, 2, 1)
npoint = len(diff6s_upper)
x = diff6s_upper
y = corrcoef12_upper
coefficients = np.polyfit(x, y, 1)
xFit = np.linspace(min(x), max(x), 1000)
yFit = np.polyval(coefficients, xFit)
#plt.scatter(x + norm.rvs(0, 0.3, size=npoint), y, s=1)
plt.scatter(x, y, s=1)
plt.plot(xFit, yFit, 'r')
plt.xlabel('firing field difference in 6s interval')
plt.ylabel('correlation coefficient for 12s interval')
plt.title('all')
...
plt.scatter(x, y, s=1)
plt.plot(xFit, yFit, 'r')
plt.xlabel('firing field difference in 12s interval')
plt.ylabel('correlation coefficient for 6s interval')
plt.title('all')
...
x = diff6s_upper
y = corrcoef12_upper
n, xedges, yedges = np.histogram2d(x, y, bins=20)
plt.imshow(n.T, origin='lower', cmap=parula,aspect='auto',interpolation='bilinear', extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
plt.colorbar()
...
plt.subplot(1, 2, 2)
x = np.round(diff12s[triu_one])
y = corrcoef6[triu_one]
n, xedges, yedges = np.histogram2d(x, y, bins=[61, 20])
plt.imshow(n.T, origin='lower',cmap=parula, aspect='auto', interpolation='bilinear',extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
plt.colorbar()
coeffs = np.polyfit(x, y, 1)
xfit = np.linspace(min(x), max(x), 100)
...
plt.xticks([r + 1.5*bar_width for r in range(len(categories))], categories)
...
# Show the plot
if save:
    fig.savefig(os.path.join(savepath,f"Sequentiality_Index"),transparent=True,dpi=600)
plt.show



#%% get lyapunov exponenents
...
def build_model_lyapunov():
    ...
    # the code below incorporated options to train input kernel within RNN layer
    hidden1=RNN(RNNCustom2sameweights_Jacobian(nUnit, 
                          output_activation=tf.keras.layers.ReLU(max_value=1000),
                          input_activation=tf.keras.layers.LeakyReLU(alpha=ReLUalpha),
                          use_bias=False,
                          kernel_initializer=initializers.RandomNormal(mean=0., stddev=1/np.sqrt(nInput), seed=seed1), # kernel initializer should be random normal
    ...
    output_A = dense_layer(hidden1[0])
    output_B = dense_layer(hidden1[1])
    Rmat=RNN(QRDcell(nUnit=nUnit,start=start),return_sequences=True)(hidden1[2])
    #output_A = Dense(2, activation='tanh',kernel_initializer=GlorotUniform(seed=seed1), kernel_constraint=IEWeightOut(nInh=nInh))(hidden1[0])
    #output_B = Dense(2, activation='tanh',kernel_initializer=GlorotUniform(seed=seed1), kernel_constraint=IEWeightOut(nInh=nInh))(hidden1[1])
...
sample_size=1
x, y, In_ons=makeInOut(sample_size,4,inputdur,nInput,min_dur,max_dur,dt)
...
dense_kernel_B=dense_kernel_A.copy()
dense_bias_B=dense_bias_A.copy()
#dense_kernel_B=model.layers[3].get_weights()[0]
#dense_bias_B=model.layers[3].get_weights()[1]

in_A=RNN_input_kernel.copy()
in_B=in_A.copy()

Wr_A, S_A=np.split(RNN_layer_Recurrent_kernel,2, axis=1)
Wr_B=Wr_A.copy()
S_B=S_A.copy()
...
#for i in np.arange(2000):
    ...
    for k in range(sample_size):
        ...
        state_A2= np.maximum(state_A[[i],:,k],0)
        state_B2= np.maximum(state_B[[i],:,k],0)
        
        hiddena=np.matmul(state_A2,Wr_A)+np.matmul(state_B2,S_B)+np.matmul(input1[[ii],:,k],in_A)+np.random.normal(loc=0,scale=0.08,size=(1,nUnit))# noise level used to be 0.08
        hiddena= np.where(hiddena > 0, hiddena, 0.2*hiddena) 
        state_A[[i+1],:,k]=(1-1/tau)*state_A[[i],:,k]+(1/tau)*hiddena
        
        
        hiddena=np.matmul(state_B2,Wr_B)+np.matmul(state_A2,S_A)+np.matmul(input1[[ii],:,k],in_B)+np.random.normal(loc=0,scale=0.08,size=(1,nUnit))
        hiddena= np.where(hiddena > 0, hiddena, 0.2*hiddena) 
        state_B[[i+1],:,k]=(1-1/tau)*state_B[[i],:,k]+(1/tau)*hiddena
        
        
        
        #calculate output
        output_A[[i+1],:,k]=np.tanh(np.matmul(state_A2,dense_kernel_A)+dense_bias_A)
        output_B[[i+1],:,k]=np.tanh(np.matmul(state_B2,dense_kernel_B)+dense_bias_B)
...
axs = axs.flatten()
for i in range(n_subplots):
    axs[i].imshow(conf_1[:,:,i], aspect='auto', cmap=parula, interpolation='none')
...
axs = axs.flatten()
for i in range(n_subplots):
    axs[i].imshow(conf_2[:,:,i], aspect='auto', cmap=parula, interpolation='none')
    axs[i].set_title(f'A: Dimension {alldim[i]}')
# Hide any unused subplots
for i in range(n_subplots, len(axs)):
    fig.delaxes(axs[i])
...
for ss in range(2):
    ...
    yticks = axs[1, ss].get_yticks()
    xticks = axs[1, ss].get_xticks()
    # Rescale tick labels
    scaled_yticks = np.round(yticks * 0.5)
    scaled_xticks = np.round(xticks * 0.01)
    # Set new tick labels
    axs[1, ss].set_yticklabels(scaled_yticks)
...
#%% get lyapunov exponent of the differences of activity

from RNNCustom2SameWeight_JacobianSynch import RNNCustom2sameweights_JacobianSynch
def build_model_lyapunov_synch():
    A_mask=build_masks(nUnit,nInh, con_prob,seed=seed1)
    B_mask=build_masks(nUnit,nInh, con_prob,seed=seed1)
    visible = Input(shape=(None,nInput)) 
    #vis_noise=GaussianNoiseAdd(stddev=0.01, seed=seed1)(visible)# used to be 0.01*np.sqrt(tau*2)
    ...
    # the code below incorporated options to train input kernel within RNN layer
    hidden1=RNN(RNNCustom2sameweights_JacobianSynch(nUnit, 
                          output_activation=tf.keras.layers.ReLU(max_value=1000),
                          input_activation=tf.keras.layers.LeakyReLU(alpha=ReLUalpha),
                          alpha=ReLUalpha,
                          use_bias=False,
...
sample_size=1
x, y, In_ons=makeInOut(sample_size,2,inputdur,nInput,min_dur,max_dur,dt)
