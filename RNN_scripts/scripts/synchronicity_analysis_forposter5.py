...
for i in range(len(weight_max)):
    ...
    foldername.append(fr"C:\Users\RHIRAsimulation\AppData\Local\anaconda3\envs\HirotoRNN3\RNN_models\t4stateful\2RNNs_prob{conpro_name}_weightmax{weight_name}_fix3")
seed=11
seed1=int(seed)
...
"""
loaded = np.load(r"C:\Users\ifumi\anaconda3\envs\myRNN1\RNNModels\t4rampprobvarfix\errormat_outputdiff_1to2.npz")
loaded.files
...
repnum=5 # number of repetitions
...
for ss in range(len(conProbability)):
    ...
    for t in range(len(nonzero_num)):# the last index is for case when there is no change added to S_A and S_B
        ...
        print(f'conprob {ss+1} out of {len(conProbability)}, loop {t+1} out of {len(nonzero_num)}')                                                            
