...
weight_max=[0.0001,0.001,0.003,0.005,0.0075,0.01,0.0125,0.015,0.0175,0.02,0.2,2]
conProbability=[0.005,0.005,0.005,0.005,0.005,0.005,0.005,0.005,0.005,0.005,0.005,0.005]
model_index=[24854,24731,19826,19045,19826,24138,15939,8861,24991,24567,24109,24859]
max_iteration=20000
...
for i in range(len(weight_max)):
    ...
    foldername.append(fr'C:\Users\RHIRAsimulation\AppData\Local\anaconda3\envs\HirotoRNN3\RNN_models\sameweights\2sameInputs_prob{conpro_name}_sameweightmax{weight_name}')
...
from RNNcustom_2_perturb_sameweights import RNNCustom2FixPerturb_sameweights
...
def build_model_perturb(nUnit,nInh,nInput,con_prob,maxval,ReLUalpha,pert_ind, pert_state,seed1):
    ...
    # the code below incorporated options to train input kernel within RNN layer
    hidden1=RNN(RNNCustom2FixPerturb_sameweights(nUnit, 
                          output_activation=tf.keras.layers.ReLU(max_value=1000),
                          input_activation=tf.keras.layers.LeakyReLU(alpha=ReLUalpha),
                          use_bias=False,
                          kernel_initializer=initializers.RandomNormal(mean=0., stddev=1/np.sqrt(nInput), seed=seed1), # kernel initializer should be random normal
...
#plt.savefig(os.path.join(r'C:\Users\RHIRAsimulation\AppData\Local\anaconda3\envs\HirotoRNN3\RNN_models',f"errormat_graph"),transparent=True,dpi=200)
...
#%%
np.savez('errormat.npz',errorMat1=errorMat1, errorMat2=errorMat2, weight_max=weight_max, conProbability=conProbability,model_index=model_index)
...
#%% get lyapunov exponent for each axis

lyap_all=np.zeros((nUnit,len(weight_max)))
#t=[4,5,6,7,8,9]
for t in range(len(weight_max)):
#for t in [0,1,2,3,4,5,6]:
    ...
    start=In_ons[0,-2]
    model_lya=build_model_lyapunov_synch()
...
for t in range(len(weight_max)):
    plt.plot(lyap_all[:,t],color=cmap(t/len(weight_max)),label=f'weight {weight_max[t]}')
    
plt.figure()
plt.plot(weight_max,lyap_all[0,:])
plt.xscale('log')
...
savedirectory=r'C:\Users\RHIRAsimulation\AppData\Local\anaconda3\envs\HirotoRNN3\RNN_models\sameweights'
os.chdir(savedirectory)
np.savez('lyapunov_exp.npz',lyap_all=lyap_all,  weight_max=weight_max, conProbability=conProbability,model_index=model_index)
