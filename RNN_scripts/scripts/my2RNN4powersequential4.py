...
for weight_ind in range(len(weight_max)):
    ...
    if save:
        ...
        directory = f"2RNNs_prob{conprobability}_weightmax{weight_name}2"      
        # Parent Directory path 
        parent_dir = r"C:\Users\RHIRAsimulation\AppData\Local\anaconda3\envs\HirotoRNN3\RNN_models\t4rampprobvar2"     
        # Path 
    ...
    mdict={"act_avg_A": act_avg_A,
          "act_avg_B": act_avg_B,}
    if save:
        os.chdir(savepath)
        savemat(f"model_{minInd}_activity_matlab.mat",mdict)
    ...
    mean_range=50
    unit_num=300
    act_avg_A_rand=act_avg_A[:,np.random.permutation(np.shape(act_avg_A)[1])]
    act_avg_B_rand=act_avg_B[:,np.random.permutation(np.shape(act_avg_B)[1])]
    PE_A, TS_A, Sql_A=Seq_ind(act_avg_A_rand[:,:unit_num],mean_range)
    PE_B, TS_B, Sql_B=Seq_ind(act_avg_B_rand[:,:unit_num],mean_range)
    ...
    if save:
        os.chdir(savepath)
        savemat(f"model_{minInd}_weights_matlab.mat",mdict)
    ...
    if save:
        plt.savefig(os.path.join(savepath,f"weight_distribution_boxplot_{minInd}"),transparent=True,dpi=600)
    plt.show()
