...
#loaded.files
loaded = np.load(r"C:\Users\RHIRAsimulation\AppData\Local\anaconda3\envs\HirotoRNN3\RNN_models\t4rampprobvarfix\errormat_outputdiff_1to6.npz")
...
loaded = np.load(r"C:\Users\RHIRAsimulation\AppData\Local\anaconda3\envs\HirotoRNN3\RNN_models\t4rampprobvarfix\errormat_outputdiff_all.npz")
errorMat1A=loaded['errorMat1A']
errorMat2A=loaded['errorMat2A']
errorMat1B=loaded['errorMat1B']
errorMat2B=loaded['errorMat2B']
...
for k_ind in range(np.shape(best_models)[0]):
    ...
    errorMat1_mean0.append(errorMat1)# shape (6,18,18,len(conprobability),2)
    errorMat2_mean0.append(errorMat2)
    AB_decode_diff_mean0.append(AB_decode_diff)# shape (6,18,18,len(conprobability),2)
    A_decode_diff_mean0.append(A_decode_diff)# shape (6,18,18,len(conprobability),2)
...
loaded_data=np.load(r"C:\Users\RHIRAsimulation\AppData\Local\anaconda3\envs\HirotoRNN3\RNN_models\Analysis_folder\prob0_state_pert.npz")

for key in loaded_data.files:
    globals()[key]=np.concatenate((loaded_data[key][:,:,:,[0],:],globals()[key]),axis=3)

#%% load data
loaded_data=np.load(r"C:\Users\RHIRAsimulation\AppData\Local\anaconda3\envs\HirotoRNN3\RNN_models\Analysis_folder\state_pert_all.npz")

for key in loaded_data.files:
    globals()[key]=np.concatenate((loaded_data[key][:,:,:,[0],:],globals()[key]),axis=3)
#%% plot the loaded data
conProb_plot=conProbability.copy()
conProb_plot[0]=1e-6
error_mean1=np.mean(errorMat1_mean0, axis=(0, 1,2)) # shape (len(weight_max),2)
error_mean2=np.mean(errorMat2_mean0, axis=(0, 1,2))
AB_decode_diff_mean=np.mean(AB_decode_diff_mean0, axis=(0, 1,2))
...
# plot output difference
plt.figure(figsize=(10, 6))
plt.plot(conProb_plot, error_mean1[:,0], marker='o', linestyle='-', label='Perturb A, Prediction 1')
plt.plot(conProb_plot, error_mean2[:,0], marker='o', linestyle='-', label='Perturb A, Prediction 2')
plt.plot(conProb_plot, error_mean1[:,1], marker='o', linestyle='-', label='Perturb B, Prediction 1')
plt.plot(conProb_plot, error_mean2[:,1], marker='o', linestyle='-', label='Perturb B, Prediction 2')
plt.xscale('log')
plt.title('Squared differences of prediction after perturbation')
plt.xlabel('Connection probability')
plt.ylabel('Squared difference')
...
# plot decoding difference between A and B after perturbation 
plt.figure(figsize=(10, 6))
plt.plot(conProb_plot, AB_decode_diff_mean[:,0], marker='o', linestyle='-', label='Perturb A')
plt.plot(conProb_plot, AB_decode_diff_mean[:,1], marker='o', linestyle='-', label='Perturb B')
...
plt.plot(conProb_plot, mean_result, marker='o', linestyle='-',color='red', linewidth=4, label='mean')
plt.plot(conProb_plot, A_decode_diff_mean[:,0], marker=None, linestyle='-', label='Decode A, Perturb A')
plt.plot(conProb_plot, A_decode_diff_mean[:,1], marker=None, linestyle='-', label='Decode A, Perturb B')
plt.plot(conProb_plot, B_decode_diff_mean[:,0], marker=None, linestyle='-', label='Decode B, Perturb A')
plt.plot(conProb_plot, B_decode_diff_mean[:,1], marker=None, linestyle='-', label='Decode B, Perturb B')
plt.plot(conProb_plot, C_decode_diff_mean[:,0], marker=None, linestyle='-', label='Decode A+B, Perturb A')
plt.plot(conProb_plot, C_decode_diff_mean[:,1], marker=None, linestyle='-', label='Decode A+B, Perturb B')
...
plt.show()
#%%
# Replace the smallest tick label with 0
ax = plt.gca()
ticks = ax.get_xticks()  # Get the tick positions
ticks[0] = x[0]  # Replace the first tick with 0
ax.set_xticks(ticks)
ax.set_xticklabels(['0' if i == 0 else (f'{ticks[i]:.0g}' if ticks[i] > x[0] else None) for i in range(len(ticks))])
...
loaded = np.load(r"C:\Users\RHIRAsimulation\AppData\Local\anaconda3\envs\HirotoRNN3\RNN_models\t4ramp_probvar\errormat_all7.npz")
...
loaded = np.load(r"C:\Users\RHIRAsimulation\AppData\Local\anaconda3\envs\HirotoRNN3\RNN_models\t4ramp_probvar\errormat_all_second.npz")
...
for t in range(len(weight_max)):
    ...
    if save:
        #plt.savefig(os.path.join(r"C:\Users\RHIRAsimulation\AppData\Local\anaconda3\envs\HirotoRNN3\RNN_models\t4ramp_probvar",f"weight_distribution_within_4"),transparent=True,dpi=600)
        plt.rcParams['svg.fonttype'] = 'none'  # Ensure text is saved as text in SVG
        plt.rcParams['text.usetex'] = False  # Disable LaTeX rendering
        # Your plotting code
...
#%% compare inter rnn connection strength
def plotdistribution(S_nonzero_A, time_diffs_A, minthreash, maxthreash, binoption='Default'):
    """
    Plots the distribution of connection strengths for two subsets of data based on time differences.
    Additionally, it plots the mean values for each distribution and assesses the statistical significance
...
lowB=[]
highB=[]
tind=np.arange(1,len(conProbability))
for t in tind:
    for k in range(np.shape(best_models)[0]):
            #  analyze weight distribution
    ...
    lowA.append(np.concatenate(subset_lowA))
    highA.append(np.concatenate(subset_highA))
    lowB.append(np.concatenate(subset_lowB))
    highB.append(np.concatenate(subset_highB))
...
for i in range(len(lowA)):
    data_pairs = [(lowA[i], highA[i]), (lowB[i], highB[i])]
    fig, axes=plotdist2_with_subplots(data_pairs, bins=100)
    fig.suptitle(f"connection probability: {conProbability[i+1]}")
    avg_lowA.append(np.mean(lowA[i]))
    std_lowA.append(np.std(lowA[i])/np.sqrt(len(lowA[i])))
    avg_lowB.append(np.mean(lowB[i]))
    std_lowB.append(np.std(lowB[i])/np.sqrt(len(lowA[i])))
    avg_highA.append(np.mean(highA[i]))
    std_highA.append(np.std(highA[i])/np.sqrt(len(lowA[i])))
    avg_highB.append(np.mean(highB[i]))
    std_highB.append(np.std(highB[i])/np.sqrt(len(lowA[i])))
    
avg_lowA=np.array(avg_lowA)
std_lowA=np.array(std_lowA)
avg_lowB=np.array(avg_lowB)
std_lowB=np.array(std_lowB)
avg_highA=np.array(avg_highA)
std_highA=np.array(std_highA)
avg_highB=np.array(avg_highB)
std_highB=np.array(std_highB)

# Create subplots
fig, axs = plt.subplots(1, 2, figsize=(12, 6), sharey=True)
x=conProbability[1:]
# Plot avg_A with std_A in the first subplot
axs[0].plot(x, avg_lowA, label=f'time_diffs_A < {minthreash}', color='blue')
axs[0].fill_between(x, avg_lowA - std_lowA, avg_lowA + std_lowA, color='blue', alpha=0.2, label='')
axs[0].plot(x, avg_highA, label=f'time_diffs_A > {maxthreash}', color='orange')
axs[0].fill_between(x, avg_highA - std_highA, avg_highA + std_highA, color='orange', alpha=0.2, label='')
axs[0].set_title('RNN A')
axs[0].set_xlabel('Connection probability')
axs[0].set_ylabel('Connection weight')
axs[0].legend()
axs[0].grid(True, linestyle='--', alpha=0.6)
axs[0].set_xscale('log')

# Plot avg_B with std_B in the second subplot
axs[1].plot(x, avg_lowB, label=f'time_diffs_B < {minthreash}', color='blue')
axs[1].fill_between(x, avg_lowB - std_lowB, avg_lowB + std_lowB, color='blue', alpha=0.2, label='')
axs[1].plot(x, avg_highB, label=f'time_diffs_B > {maxthreash}', color='orange')
axs[1].fill_between(x, avg_highB - std_highB, avg_highB + std_highB, color='orange', alpha=0.2, label='')
axs[1].set_title('RNN B')
axs[1].set_xlabel('Connection probability')
axs[1].set_ylabel('Connection weight')
axs[1].legend()
axs[1].grid(True, linestyle='--', alpha=0.6)
axs[1].set_xscale('log')    

# Adjust layout
fig.suptitle('Comparison of Averages with Standard Deviations', fontsize=16)
fig.tight_layout()
fig.subplots_adjust(top=0.88)  # Leave space for the global title

#%% show sequential activity and single unit activity
...
def sort_and_scale2(act_avg_A,start,stop):
    # act_avg_A is matrix with size (time, nCells)
    # this function first sort cells accorcding to the time of maximum activity from start to stop
    # then this function scale the value so that the maximum is 1.
    act_avg_A=np.array(act_avg_A)
...
for k in range(np.shape(best_models)[0]):
    for t in range(len(weight_max)):
        ...
        act_avg_A_max=sort_and_scale(act_avg_A,min_dur,min_dur+max_dur)
        act_avg_B_max=sort_and_scale(act_avg_B,min_dur,min_dur+max_dur)
        act_avg_A_min=sort_and_scale(act_avg_A,0,min_dur)
        act_avg_B_min=sort_and_scale(act_avg_B,0,min_dur)
        
        # get max time
        min_A.append(np.argmax(act_avg_A[0:min_dur,:],axis=0))
        min_B.append(np.argmax(act_avg_B[0:min_dur,:],axis=0))
        max_A.append(np.argmax(act_avg_A[min_dur:min_dur+max_dur,:],axis=0))
        max_B.append(np.argmax(act_avg_B[min_dur:min_dur+max_dur,:],axis=0))

        # plot activities
        fig, axs = plt.subplots(2,2,sharex=True, sharey=True,figsize=(15,30))
        axs[0,0].imshow(act_avg_A_max.T, aspect='auto', cmap=parula,interpolation='none', vmin=0, vmax=1)
        axs[0,0].set_title(f'A:max')
        axs[0,0].set_xticks([0,600,1200,1800])
        axs[0,0].set_xticklabels([0,6,12,18])
        axs[0,1].imshow(act_avg_B_max.T, aspect='auto', cmap=parula,interpolation='none', vmin=0, vmax=1)
        axs[0,1].set_title(f'B:max')
        axs[0,1].spines['left'].set_visible(False)
        axs[0,1].tick_params(left=False, labelleft=False)
        axs[0,1].set_xticks([0,600,1200,1800])
        axs[0,1].set_xticklabels([0,6,12,18])
        axs[1,0].imshow(act_avg_A_min.T, aspect='auto', cmap=parula,interpolation='none', vmin=0, vmax=1)
        axs[1,0].set_title(f'A:min')
        axs[1,0].set_xticks([0,600,1200,1800])
        axs[1,0].set_xticklabels([0,6,12,18])
        axs[1,1].imshow(act_avg_B_min.T, aspect='auto', cmap=parula,interpolation='none', vmin=0, vmax=1)
        axs[1,1].set_title(f'B:min')
        axs[1,1].spines['left'].set_visible(False)
        axs[1,1].tick_params(left=False, labelleft=False)
        axs[1,1].set_xticks([0,600,1200,1800])
        axs[1,1].set_xticklabels([0,6,12,18])
        fig.suptitle(f'conprob:{conProbability[t]}, loop:{k}', fontsize=32)
        plt.show()


# plot maxtime
min_A=np.concatenate(min_A,0)
max_A=np.concatenate(max_A,0)
min_B=np.concatenate(min_B,0)
max_B=np.concatenate(max_B,0)
fig, axs = plt.subplots(2, 1,figsize=(15,8))
axs[0].scatter(max_A,min_A,1)
axs[0].plot([0,min_dur],[0,min_dur],color="orange")
axs[0].plot([0,max_dur],[0,min_dur],color="green")
axs[0].set_aspect('equal', 'box')
axs[0].set_xticks([0,600,1200])
axs[0].set_xticklabels([0,6,12])
axs[0].set_yticks([0,300,600])
axs[0].set_yticklabels([0,3,6])


axs[1].scatter(max_B,min_B,1)
axs[1].plot([0,min_dur],[0,min_dur],color="orange")
axs[1].plot([0,max_dur],[0,min_dur],color="green")
axs[1].set_aspect('equal', 'box')
axs[1].set_xticks([0,600,1200])
axs[1].set_xticklabels([0,6,12])
axs[1].set_yticks([0,300,600])
axs[1].set_yticklabels([0,3,6])

#%% visualize single unit activity
t=2
k=5
max_range=[0,max_dur+min_dur] # time range to choose max firing time
nExc=nUnit-nInh #number of excitory units
...
act_avg_A_max=sort_and_scale2(act_avg_A,min_dur,min_dur+max_dur)
act_avg_B_max=sort_and_scale2(act_avg_B,min_dur,min_dur+max_dur)
act_avg_A_min=sort_and_scale2(act_avg_A,0,min_dur)
act_avg_B_min=sort_and_scale2(act_avg_B,0,min_dur)

# plot activities
fig, axs = plt.subplots(2,2,sharex=True, sharey=True,figsize=(15,30))
axs[0,0].imshow(act_avg_A_max[0].T, aspect='auto', cmap=parula,interpolation='none', vmin=0, vmax=1)
axs[0,0].set_title(f'A:max')
axs[0,0].set_xticks([0,600,1200,1800])
axs[0,0].set_xticklabels([0,6,12,18])
axs[0,1].imshow(act_avg_B_max[0].T, aspect='auto', cmap=parula,interpolation='none', vmin=0, vmax=1)
axs[0,1].set_title(f'B:max')
axs[0,1].spines['left'].set_visible(False)
axs[0,1].tick_params(left=False, labelleft=False)
axs[0,1].set_xticks([0,600,1200,1800])
axs[0,1].set_xticklabels([0,6,12,18])
axs[1,0].imshow(act_avg_A_min[0].T, aspect='auto', cmap=parula,interpolation='none', vmin=0, vmax=1)
axs[1,0].set_title(f'A:min')
axs[1,0].set_xticks([0,600,1200,1800])
axs[1,0].set_xticklabels([0,6,12,18])
axs[1,1].imshow(act_avg_B_min[0].T, aspect='auto', cmap=parula,interpolation='none', vmin=0, vmax=1)
axs[1,1].set_title(f'B:min')
axs[1,1].spines['left'].set_visible(False)
axs[1,1].tick_params(left=False, labelleft=False)
axs[1,1].set_xticks([0,600,1200,1800])
...
# plot maxtime
min_A=np.argmax(act_avg_A[0:min_dur,:],axis=0)
min_B=np.argmax(act_avg_B[0:min_dur,:],axis=0)
max_A=np.argmax(act_avg_A[min_dur:min_dur+max_dur,:],axis=0)
max_B=np.argmax(act_avg_B[min_dur:min_dur+max_dur,:],axis=0)

fig, axs = plt.subplots(2, 1,figsize=(15,8))
axs[0].scatter(max_A,min_A)
axs[0].plot([0,min_dur],[0,min_dur])
axs[0].plot([0,max_dur],[0,min_dur])
axs[0].set_aspect('equal', 'box')
axs[1].scatter(max_B,min_B)
axs[1].plot([0,min_dur],[0,min_dur])
axs[1].plot([0,max_dur],[0,min_dur])
axs[1].set_aspect('equal', 'box')

unit_num=281
showdata=act_avg_A_max
fig, axs = plt.subplots(1, 1,figsize=(15,8))
axs.plot(act_avg_A[:,showdata[1][unit_num]])
axs.set_xticks([0,600,1200,1800])
axs.set_xticklabels([0,6,12,18])
...
for t in tind:
    ...
    #for t in aa:
        ...
        pred_Call_2 = pred_Call[0].flatten(order='F').reshape(-1, 1)
        ...
        diff_C=get_phase(pred_Call_2-class_all,class_per_trial,'int')
        # get offset from the actual decoded results
        pred_diff_real_sub.append([np.linalg.norm(diff_A),np.linalg.norm(diff_B),np.linalg.norm(diff_C)])
    ...
    pred_diff_real= np.mean(pred_diff_real_sub,  axis=0) #(6,3)-> (3,)
    ...
    Allinfo['pred_diff_sub'].append(pred_diff_sub)
    Allinfo['pred_diff_real'].append(pred_diff_real)
...
fig,axs=plt.subplots(1,1)
axs.plot(x,np.array(Allinfo['pred_diff_real'])[:,0],label='Prediction by A')
axs.plot(x,np.array(Allinfo['pred_diff_real'])[:,1],label='Prediction by B')
axs.plot(x,np.array(Allinfo['pred_diff_real'])[:,2],label='Prediction by A&B')
axs.legend()
axs.set_xscale('log')
axs.set_title('Decoded results and true time difference')
...
json_file_path = os.path.join(analysis_folder, f"Allinfo_include0_pert_noisesd_{pert_noisesd}_pert_prob_{pert_prob}.json")
...
#%% perturb with noise then decode-> get dimensionality
...
def perturb_and_decode_noise_prob(trial1,ind1,pert_which,order,pca_A,pca_B,pca_C,clf_A,clf_B,clf_C,pert_noisesd,stop):
    ...
    for i in np.arange(np.shape(actpart_A)[2]):
        #dimension reduction
        proj_A=pca_A.transform(actpart_A[:,:,i])# time, ncells, trials
        proj_B=pca_B.transform(actpart_B[:,:,i])
        proj_C=pca_C.transform(actpart_C[:,:,i])
        ...
        accum_A.append(proj_A)
        accum_B.append(proj_B)
        
    # perform pca and get dimentionality
    accum_A=np.concatenate(accum_A,axis=0)# time, ncells
    accum_B=np.concatenate(accum_B,axis=0)
    accum_C=np.concatenate((accum_A,accum_B),axis=1)
    pca_dim_A = PCA()
    pca_dim_B = PCA()
    pca_dim_C = PCA()
    
    # zscore and fit pca models
    accum_A = z_score_with_zero_handling(accum_A,dim=0)
    accum_B = z_score_with_zero_handling(accum_B,dim=0)
    accum_C = z_score_with_zero_handling(accum_C,dim=0)
    
    pca_dim_A.fit(accum_A)
    pca_dim_B.fit(accum_B)
    pca_dim_C.fit(accum_C)
    
    # get variance explained
    var_exp=[]# (3,) list with each being (nCell,) or (2*nCell,)
    var_exp.append(pca_dim_A.explained_variance_ratio_)
    var_exp.append(pca_dim_B.explained_variance_ratio_)
    var_exp.append(pca_dim_C.explained_variance_ratio_)
    
    return pred_A, pred_B, pred_C, predictions2, pert_ind_2, In_ons_2, var_exp
    
sample_size=12
trial_num=8
pert_state=0 # 0 to perturb RNN A and 1 to perturb RNN B
pert_noisesd=2.5#1.5 original 0.8
stop=False
# take circular mean of the prediction
option=0 #0 for circular, 1 for mean
trial1=2
...
Allinfo = {
    't_index':         [],
    'confA_list':      [],  # Raw confA_list
    'confB_list':      [],  # Raw confB_list
    'offset_list':     [],  # Raw offset_list
    'terror_list':     [],  # Raw terror_list
    'caterr_list':     [],  # Raw caterr_list
    'pred_diff2':      [],  # Raw pred_diff2
    'pred_diff_real_sub': [],  # Raw pred_diff_real_sub
    'var_exp_sub':         [],
...
for t in tind:
    ...
    #for t in aa:
        ...
        #perturb A
        pert_state=0
        pred_A, pred_B, pred_C, A_predictions2, pert_ind_3, In_ons_a, var_exp_sub2 =perturb_and_decode_noise_prob(trial1,time_1,pert_which,order,pca_A,pca_B,pca_C,clf_A,clf_B,clf_C,pert_noisesd,stop)
        predictions2.append(A_predictions2)
        pert_ind_2.append(pert_ind_3)
        In_ons_2.append(In_ons_a)
        class_all=np.tile(class_A,int((trial_num-trial1)/2))
        var_exp_sub.append(var_exp_sub2) # (6,3,cells)
    ...
    # Initialize a list to store the averaged vectors
    var_exp_avg = []

    # Iterate over the components (3, 4, and 5-sized vectors)
    for idx in range(len(var_exp_sub[0])):  # Length is the number of vector groups (3)
        # Collect all vectors of the same group from all `V[i]`
        group_vectors = [var_exp_sub[i][idx] for i in range(len(var_exp_sub))]
        # Stack and compute the mean along the first dimension
        averaged_vector = np.mean(np.stack(group_vectors), axis=0)
        var_exp_avg.append(averaged_vector)
        
    # Append both raw lists and averages to the dictionary
    Allinfo['t_index'].append(t)
    
    # Append raw lists
    Allinfo['confA_list'].append(confA_list)
    Allinfo['confB_list'].append(confB_list)
    Allinfo['offset_list'].append(offset_list)
    Allinfo['terror_list'].append(terror_list)
    Allinfo['caterr_list'].append(caterr_list)
    Allinfo['pred_diff2'].append(pred_diff2)
    Allinfo['pred_diff_real_sub'].append(pred_diff_real_sub)
    Allinfo['var_exp_sub'].append(var_exp_sub)
    ...
    Allinfo['pred_diff_sub'].append(pred_diff_sub)
    Allinfo['pred_diff_real'].append(pred_diff_real)
    Allinfo['var_exp_avg'].append(var_exp_avg)
...
for t in range(len(conProbability)):
    ...
    # Plot the categorical data as a line plot
    axs[0].plot(catlabel, Allinfo['cat_error_ave'][t], marker='o', color=color, label=f'Con Prob {conProbability[t]:.1e}')
    axs[0].set_title('Categorical Error')
    axs[0].legend()

    # Plot temporal error data
    axs[1].plot(offset_temp, Allinfo['temp_error_ave'][t], color=color, label=f'Con Prob {conProbability[t]:.1e}')
    axs[1].set_title('Temporal Error')
    axs[1].legend()
...
for i in range(len(conProbability)):
    va_A=Allinfo['var_exp_avg'][i][0]
    va_B=Allinfo['var_exp_avg'][i][1]
    va_C=Allinfo['var_exp_avg'][i][2]
    
    va_A=np.cumsum(va_A)
    va_B=np.cumsum(va_B)
    va_C=np.cumsum(va_C)
    va_all=[va_A,va_B,va_C]
    for t,ax in enumerate(axes):
        ax.plot(np.arange(1,dim_num+1),va_all[t][:dim_num], color=colors[i], label=f'conProb: {conProbability[i]:.1e}')
        ax.set_xlabel('PCs')
        #ax.set_xscale('log')
        ax.legend()
axes[0].set_ylabel('Variance explained')
axes[0].set_title("RNN A")
axes[1].set_title("RNN B")
axes[2].set_title("RNN A&B")
...
#%% perturb with noise + noise on one unit-> then decode-> get dimensionality

#build model
from RNNcustom_2_perturb_noise_prob import RNNCustom2FixPerturb_noise_prob_unitnoise
def build_model_perturb_noise_prob_unit(nUnit,nInh,nInput,con_prob,maxval,ReLUalpha,pert_ind,pert_which, seed1, pert_noisesd,unit_noise_ind, unit_noise_A, unit_noise_SD):
    A_mask=build_masks(nUnit,nInh, con_prob,seed=seed1)
    B_mask=build_masks(nUnit,nInh, con_prob,seed=seed1)
    visible = Input(shape=(None,nInput)) 
    #vis_noise=GaussianNoiseAdd(stddev=0.01, seed=seed1)(visible)# used to be 0.01*np.sqrt(tau*2)
    ...
    # the code below incorporated options to train input kernel within RNN layer
    hidden1=RNN(RNNCustom2FixPerturb_noise_prob_unitnoise(nUnit, 
                          output_activation=tf.keras.layers.ReLU(max_value=1000),
                          input_activation=tf.keras.layers.LeakyReLU(alpha=ReLUalpha),
                          use_bias=False,
                          kernel_initializer=initializers.RandomNormal(mean=0., stddev=1/np.sqrt(nInput), seed=seed1), # kernel initializer should be random normal
...
def perturb_and_decode_noise_prob(trial1,ind1,pert_which,order,pca_A,pca_B,pca_C,clf_A,clf_B,clf_C,pert_noisesd,stop,unit_noise_ind,unit_noise_A,unit_noise_SD):
    x, y, In_ons=makeInOut_sameint(sample_size,trial_num,inputdur,nInput,min_dur,max_dur,dt,order)
    ...
    #pert_ind=np.zeros((4,2))
    #model2=build_model_perturb(pert_ind, pert_state)
    model2=build_model_perturb_noise_prob_unit(nUnit=nUnit, 
                               nInh=nInh, 
                               nInput=nInput, 
                               con_prob=con_prob, 
                               maxval=maxval ,
                               ...
                               seed1=seed1,
                               pert_noisesd=pert_noisesd,
                               unit_noise_ind=unit_noise_ind, #index of unit to apply noise
                               unit_noise_A=unit_noise_A,# 0 to apply unit noise to RNN A and 1 otherwise
                               unit_noise_SD=unit_noise_SD# SD of unit noise)
                               )
    model2.set_weights(model.get_weights())
    predictions = model2.predict(x)
...
trial_num=8
pert_state=0 # 0 to perturb RNN A and 1 to perturb RNN B
pert_noisesd=1.5#1.5 original 0.8
unit_noise_SD=2.0
stop=False
# take circular mean of the prediction
option=0 #0 for circular, 1 for mean
...
for t in tind:
    ...
    #for t in aa:
        ...
        pred_A, pred_B, pred_C, A_predictions2, pert_ind_3, In_ons_a, var_exp_sub2 =perturb_and_decode_noise_prob(trial1,
                                                                                                                  time_1,
                                                                                                                  pert_which,
                                                                                                                  order,
                                                                                                                  pca_A,
...
Allinfo['pert_prob']=pert_prob
Allinfo['pert_noisesd']=pert_noisesd
Allinfo['unit_noise_A']=unit_noise_A
Allinfo['unit_noise_SD']=unit_noise_SD
Allinfo['unit_noise_ind']=unit_noise_ind
Allinfo['xedges']=xedges
Allinfo['yedges']=yedges
Allinfo['offset_temp']=offset_temp
Allinfo['tind']=tind
...
def plot_matrix_2d(matrix, ax, k, m):
    ...
    for i in range(k):
        ...
        colors = hsv(np.linspace(0, 1, m))
        # Scatter the points in 3D space with assigned colors
        ax.scatter(segment[:, 0], segment[:, 1], 2,c=colors, marker='o')
...
def avgAct2(activities,In_ons,min_dur,max_dur):
    ...
    act_avg=np.zeros((min_dur+max_dur,np.shape(activities)[2]))
    for i in np.arange(np.shape(activities)[0]):
        for j in range(2,np.shape(In_ons)[1]-1):
            if In_ons[i,j+1]-In_ons[i,j]<=avg_dur and In_ons[i,j]+min_dur+max_dur<=np.shape(activities)[1]:
                act_avg+=np.squeeze(activities[i,In_ons[i,j]:In_ons[i,j]+min_dur+max_dur,:])
                kk=kk+1
    act_avg/=kk
    return act_avg
...
for k in range(np.shape(best_models)[0]):
    ...
    #for t in aa:
        ...
        model=build_model(nUnit=nUnit, nInh=nInh, nInput=nInput, con_prob=con_prob, maxval=maxval ,ReLUalpha=ReLUalpha, seed1=seed1)
        # load weights
        model.load_weights(best_models[k][t])
        ...
        activities_A = output_and_activities[0]  # Activities of all intermediate layers
        activities_B=output_and_activities[1]
        act_avg_A=avgAct2(activities_A,In_ons,min_dur,max_dur)
        act_avg_B=avgAct2(activities_B,In_ons,min_dur,max_dur)
        act_stack_A=Act_2dsort(activities_A,In_ons,min_dur,max_dur)#(time,nUnit) time is multiple of mindur+maxdur
        act_stack_B=Act_2dsort(activities_B,In_ons,min_dur,max_dur)
...
#%% plot trajectories of two rnns together
...
pearson_mat=[]
pearson_avg=[]
fig, axs = plt.subplots(1,len(conProbability))
for t in range(len(conProbability)):
    dis_mat=[]
    for k in range(np.shape(best_models)[0]):
    #for t in aa:
        ...
        # Get the output and activities of all layers for the new input data
        #x, y, In_ons=makeInOut(2,8,inputdur,nInput,min_dur,max_dur,dt)
        x, y, In_ons=makeInOut_sameint(2,8,inputdur,nInput,min_dur,max_dur,dt,0)
        #xnew=sum(xnew,axis=2)
        output_and_activities = activity_model.predict(x)
        activities_A = output_and_activities[0]  # Activities of all intermediate layers
        activities_B=output_and_activities[1]
        ...
        act_avg=np.concatenate((act_avg_A,act_avg_B),axis=1)
        # make correlation matrix
        act_avg0=act_avg-np.mean(act_avg,axis=0) # 1800*1024
        dotpro0=np.matmul(act_avg0,act_avg0.T) # dissimilarity matrix using dot product
        pearsoncorr=np.corrcoef(act_avg0,rowvar=True) # (1800,1800) pearson correlation coefficients
        dis_mat0=scipy.spatial.distance_matrix(act_avg,act_avg) #1800,1800 distant matrix 
        dis_mat.append(dotpro0)
        pearson_mat.append(pearsoncorr)
        pca_C = PCA()
        pca_C.fit(act_avg)
        proj_C_train=pca_C.transform(act_avg)
        
        
        
    dis_mat_avg=np.mean(dis_mat,axis=0)#1800,1800 distant matrix
    dissim_mat.append(dis_mat_avg)
    
    pearson_avg.append(np.mean(pearson_mat,axis=0))
    
    # mds-> do svd
    svd_result=np.linalg.svd(dis_mat_avg,hermitian=True)
    
    dimnum=2
    m=min_dur+max_dur
    recon_data=svd_result[0][:,:dimnum]*np.sqrt(svd_result[1][:dimnum])# 1800,dimnum
    plot_matrix_2d(recon_data, axs[t], 1, m)
    
    dissim_trans.append(recon_data)

fig, axs = plt.subplots(1,len(conProbability), figsize=(6*len(conProbability), 6))
for t in range(len(conProbability)):
    plot_matrix_2d(dissim_trans[t], axs[t], 1, m)
fig.tight_layout()
if save:
    #plt.savefig(os.path.join(savepath,f"Perturb_decode_{time_1}_{time_2}_{order}_{option}"),transparent=True,dpi=400)
    plt.rcParams['svg.fonttype'] = 'none'  # Ensure text is saved as text in SVG
    ...
    # Your plotting code
    plt.savefig(os.path.join(figure_folders,f"MDS_reconstruction_2.svg"), format='svg')
    plt.savefig(os.path.join(figure_folders,f"MDS_reconstruction_2.png"), transparent=True,dpi=500)
plt.show()    


fig, axs = plt.subplots(1,len(conProbability), figsize=(6*len(conProbability), 6),sharey=True)
vmin=0
vmax=1500
for t in range(len(conProbability)):
    im=axs[t].imshow(dissim_mat[t],aspect='auto', cmap=parula, interpolation='none',vmin=vmin,vmax=vmax)
    axs[t].set_aspect('equal')
    axs[t].set_title(f'conprob: {conProbability[t]}')
...
if save:
    ...
    # Your plotting code
    plt.savefig(os.path.join(figure_folders,f"Dissim_avg_2.svg"), format='svg')
    plt.savefig(os.path.join(figure_folders,f"Dissim_avg_2.png"), transparent=True,dpi=500)
plt.show() 
...
vmax=1
for t in range(len(conProbability)):
    im=axs[t].imshow(pearson_avg[t],aspect='auto', cmap=parula, interpolation='none',vmin=vmin,vmax=vmax)
    axs[t].set_aspect('equal')
    axs[t].set_title(f'conprob: {conProbability[t]}')
#fig.colorbar(im, orientation='vertical')
fig.tight_layout()
...
if save:
    ...
    # Your plotting code
    plt.savefig(os.path.join(figure_folders,f"pearson_avg_2.svg"), format='svg')
    plt.savefig(os.path.join(figure_folders,f"pearson_avg_2.png"), transparent=True,dpi=500)
plt.show() 
...
for t in range(len(conProbability)):
    ...
    transformed=MDSembed.fit_transform(dissim_mat[t])
    m=min_dur+max_dur
    plot_matrix_2d(dissim_trans[t], axs[t], 1, m)
    axs[t].set_aspect('equal')
    axs[t].set_title(f'conprob: {conProbability[t]}')
...
for t in range(len(conProbability)):
    MDSembed=MDS(n_components=2, dissimilarity='precomputed')
    transformed=MDSembed.fit_transform(1-pearson_avg[t])
    m=min_dur+max_dur
    plot_matrix_2d(dissim_trans[t], axs[t], 1, m)
    axs[t].set_aspect('equal')
    axs[t].set_title(f'conprob: {conProbability[t]}')
#%% save data
save_name=os.path.join(analysis_folder,'Distance_matrix_2')
np.savez(save_name,dissim_trans=dissim_trans, dissim_mat=dissim_mat,pearson_avg=pearson_avg)

#%% load the data
data=np.load(os.path.join(analysis_folder,'Distance_matrix.npz'))
#%%
    
    # MDS
    embedding = MDS(n_components=2, dissimilarity='precomputed')
    dis_mat_transformed = embedding.fit_transform(dis_mat_avg) #1800,2
    dissim_trans.append(dis_mat_transformed)
    m=min_dur+max_dur
        # Plot matrix A and B
    plot_matrix_2d(dis_mat_transformed, axs[t], 1, m)
...
def makeInOut_sameint2(sample_size,trial_num,inputdur,nInput,min_dur,max_dur,dt,interval):
    # make intervals always start from min_dur or max_dur
    # make inputs and outputs
    # there are 2 kinds of input and for each input, output is a slowly increasing activity
    #total_time=100+2*inputdur+max_dur+100
...
#%% get lyapunov exponent of the entire activities

from RNNcustom_2_fix_2_jacobian import RNNCustom2Fix2_jacobian
from QRDCell2 import QRDcell2
def build_model_lyapunov():
    A_mask=build_masks(nUnit,nInh, con_prob,seed=seed1)
    B_mask=build_masks(nUnit,nInh, con_prob,seed=seed1)
    visible = Input(batch_shape=(batch_size, None, nInput))
    #vis_noise=GaussianNoiseAdd(stddev=0.01, seed=seed1)(visible)# used to be 0.01*np.sqrt(tau*2)
    #hidden1 = SimpleRNN(nUnit,activation='tanh', use_bias=False, batch_size=batch_sz, stateful=False, input_shape=(None, 1), return_sequences=True)(vis_noise)
    ...
    rnn_cell = RNNCustom2Fix2_jacobian(
        nUnit,
        output_activation=tf.keras.layers.ReLU(max_value=1000),
        input_activation=tf.keras.layers.LeakyReLU(alpha=ReLUalpha),
        use_bias=False,
    ...
    hidden1 = rnn_layer(visible, initial_state=[initial_state_A, initial_state_B])
    

    #  hidden2 = Dense(10, activation='relu')(hidden1)
    
    dense_layer = Dense(2, activation='tanh',kernel_initializer=GlorotUniform(seed=seed1), kernel_constraint=IEWeightOut(nInh=nInh))
    # Apply the Dense layer to each LSTM output separately
    ...
    Rmat=RNN(QRDcell2(nUnit=nUnit,start=start),return_sequences=True)(hidden1[2])# (batch_num,timestep,2*nUnits)
    #output_A = Dense(2, activation='tanh',kernel_initializer=GlorotUniform(seed=seed1), kernel_constraint=IEWeightOut(nInh=nInh))(hidden1[0])
    #output_B = Dense(2, activation='tanh',kernel_initializer=GlorotUniform(seed=seed1), kernel_constraint=IEWeightOut(nInh=nInh))(hidden1[1])
    output=[Concatenate(axis=2)([output_A,output_B]),Rmat]
...
sample_size=2
batch_size=2
x, y, In_ons=makeInOut(batch_size,6,inputdur,nInput,min_dur,max_dur,dt)


start=1000
model_lya=build_model_lyapunov()
print(model_lya.summary())
checkpoint_filepath=best_models[0][0]
model_lya.load_weights(checkpoint_filepath)

#model_lya.set_weights(model.get_weights())
#model_lya([tf.cast(x,dtype=tf.float32),tf.zeros([batch_size,nUnit],tf.float32),tf.zeros([batch_size,nUnit],tf.float32)])
predictions =model_lya.predict([x,np.zeros((batch_size,nUnit)),np.zeros((batch_size,nUnit))])
Rtri=tf.math.abs(predictions[1][:,start:,:])

#calculate lyapunov exponent
Rtri=tf.math.log(Rtri)
Rtri=tf.math.cumsum(Rtri,1)
# take average
timepoints=tf.shape(Rtri)[1]
v = tf.reshape(tf.range(tf.cast(timepoints,dtype=tf.float32), dtype=tf.float32), (1, timepoints, 1))

# Perform the element-wise division
lyapunov_exp = Rtri / v
plt.figure()
for k in range(sample_size):
    plt.plot(lyapunov_exp[k,-1,:])
plt.show()
lyapunov_avg=tf.math.reduce_mean(lyapunov_exp[:,-1,:],axis=0)


normPred=predictions[0]
fig, axs = plt.subplots(2, 1, figsize=(14, 8))

# Plotting data for each subplot
for k in range(2):
    axs[k].plot(y[k, :, 0], color='blue')
    axs[k].plot(normPred[k, :, 0], color='green')
    axs[k].plot(y[k, :, 1], color='red')
    axs[k].plot(normPred[k, :, 1], color='orange')                
    axs[k].set_title(f'Plot')
# Adjusting layout
plt.tight_layout()
plt.show() 
...
#%% get lyapunov exponent of the entire activities-> use stateful +flatten matrices to pass on
# first make input and output stateful
import gc
def makeInOutstateful(sample_size,trial_num,inputdur,nInput,min_dur,max_dur,dt,batch_num):
    # make inputs and outputs
    # there are 2 kinds of input and for each input, output is a slowly increasing activity
    #total_time=100+2*inputdur+max_dur+100
    noise_range=0 #level of temporal noise with respect to duration
...
from RNNcustom_2_fix_2_jacobian import RNNCustom2Fix2_jacobian_flat
from QRDCell2 import QRDcell2_flat
def build_model_lyapunov():
    A_mask=build_masks(nUnit,nInh, con_prob,seed=seed1)
    B_mask=build_masks(nUnit,nInh, con_prob,seed=seed1)
    visible = Input(batch_shape=(batch_size, None, nInput))
    ...
    # Create your custom RNN cell
    rnn_cell = RNNCustom2Fix2_jacobian_flat(
        nUnit,
        output_activation=tf.keras.layers.ReLU(max_value=1000),
        input_activation=tf.keras.layers.LeakyReLU(alpha=ReLUalpha),
        use_bias=False,
    ...
    output_B = Dense(2, activation='tanh',
                     kernel_initializer=GlorotUniform(seed=seed1),
                     kernel_constraint=IEWeightOut(nInh=nInh))(hidden1[1])
    QRDCell=QRDcell2_flat(nUnit=nUnit,start=start)
    #hidden1[2] = tf.ensure_shape(hidden1[2], [batch_size, None, 4 * nUnit**2])
    Rlayer=RNN(QRDCell,stateful=True,return_sequences=True)# (batch_num,timestep,2*nUnits)
    Rmat=Rlayer(hidden1[2])
    #output_A = Dense(2, activation='tanh',kernel_initializer=GlorotUniform(seed=seed1), kernel_constraint=IEWeightOut(nInh=nInh))(hidden1[0])
    #output_B = Dense(2, activation='tanh',kernel_initializer=GlorotUniform(seed=seed1), kernel_constraint=IEWeightOut(nInh=nInh))(hidden1[1])
    output=[Concatenate(axis=2)([output_A,output_B]),Rmat]
    model = Model(inputs=[visible, initial_state_A, initial_state_B], outputs=output)
...
batch_size=1
batch_num=10
x, y, In_ons=makeInOut(batch_size,4,inputdur,nInput,min_dur,max_dur,dt)
x, y, In_ons, Wbin=makeInOutstateful(batch_size,3,inputdur,nInput,min_dur,max_dur,dt,batch_num)
...
for i in range(len(x)):
    ...
    predictions =model_lya.predict_on_batch([xin,tf.zeros((batch_size,nUnit)),tf.zeros((batch_size,nUnit))])
    ramp_out.append(predictions[0])
    lyapunov_exp.append(np.mean(predictions[1],0))# append (time,1024)
    
    # Clear the session and collect garbage
    _ = gc.collect()    
    
ramp_out=np.concatenate(ramp_out,axis=1)
lyapunov_exp=np.concatenate(lyapunov_exp,axis=0)# append (timesteps,1024)
...
for k in range(4):
    ...
    axs[k].plot(ramp_out[0, :, k], color='green')            
    #axs[k].set_title(f'Plot {i+1}')
# Adjusting layout
plt.tight_layout()
plt.show()

# get lyapunov exponenent
Rtri=tf.math.abs(lyapunov_exp[start+1:,:])#  (timesteps,1024)

#calculate lyapunov exponent
Rtri=tf.math.log(Rtri)#  (timesteps,1024)
Rtri=tf.math.cumsum(Rtri,0)#  (timesteps,1024)
# take average
timepoints=tf.shape(Rtri)[0]
#v = tf.reshape(tf.range(tf.cast(timepoints,dtype=tf.float32), dtype=tf.float32), (1, timepoints, 1))
v=tf.reshape(tf.cast(tf.range(timepoints),dtype=tf.float32), (-1, 1))
# Perform the element-wise division
lyapunov_exp_qr = Rtri / v#  (timesteps,1024)
plt.figure()
plt.plot(lyapunov_exp_qr[-1,:])

# display sorted lyapunov exponent
sorted_indices = tf.argsort(lyapunov_exp_qr[-1, :], direction="DESCENDING")
reordered_lyapunov_exp_qr = tf.gather(lyapunov_exp_qr, sorted_indices, axis=1)
plt.figure()
plt.plot(reordered_lyapunov_exp_qr [-1,:])

#%% get lyapunov exponenet for all of the RNNs
batch_size=1
batch_num=10
x, y, In_ons, Wbin=makeInOutstateful(batch_size,4,inputdur,nInput,min_dur,max_dur,dt,batch_num)
start=In_ons[0][2]
lyapunov_all=[]
for t in range(len(conProbability)):
    lyapunov_sub=[]
    for k in range(np.shape(best_models)[0]):
        ...
        # Plotting data for each subplot
        for k in range(4):
            axs[k].plot(ycat[0, :, k], color='blue')
            axs[k].plot(ramp_out[0, :, k], color='green')            
            #axs[k].set_title(f'Plot {i+1}')
        # Adjusting layout
        plt.tight_layout()
        plt.title(f"k:{k}, conProb:{conProbability[t]}")
        plt.show()
        ...
        v=tf.reshape(tf.cast(tf.range(timepoints),dtype=tf.float32), (-1, 1))
        # Perform the element-wise division
        lyapunov_exp_qr = Rtri / (v+1)#  (timesteps,1024)
        plt.figure()
        plt.plot(lyapunov_exp_qr[-1,:])
        ...
        plt.plot(reordered_lyapunov_exp_qr [-1,:])
        plt.title(f"k:{k}, conProb:{conProbability[t]}")
        lyapunov_sub.append(lyapunov_exp_qr)#  (6,timesteps,1024)
        print(f"k:{k}, t:{t}")
    lyapunov_all.append(np.mean(lyapunov_sub,0))#  (conprobability,timesteps,1024)

#%% display laypunov exponents
import matplotlib.cm as cm
...
for t in range(len(conProbability)):
    color = colors[t]
    # Plot the categorical data as a line plot
    axs.plot(lyapunov_all[t][-1,:], marker=None, color=color, label=f'Con Prob {conProbability[t]:.2f}')
axs.set_title('Lyapunov spectrum')
axs.set_xlabel('Dimension')
axs.set_ylabel('Lyapunov exponent')
axs.legend()
#plt.suptitle(f'Connection Probability {conProbability[t]:.2f}')
#plt.tight_layout()
plt.show()
...
# plot ordered lyapunov exponent
reordered_lyapunov=[]
largest_lyapunov=[]
for t in range(len(conProbability)):
    sorted_indices = tf.argsort(lyapunov_all[t][-1,:], direction="DESCENDING")
    reordered_lyapunov.append(tf.gather(lyapunov_all[t], sorted_indices, axis=1))
reordered_lyapunov=np.array(reordered_lyapunov)#(conprobability, timesteps, 2*nUnits)   
fig, axs = plt.subplots(1, 1, figsize=(10, 10))
for t in range(len(conProbability)):
    color = colors[t]
    # Plot the categorical data as a line plot
    axs.plot(reordered_lyapunov[t,-1,:], marker=None, color=color, label=f'Con Prob {conProbability[t]:.2f}')
...
#plt.tight_layout()
plt.show()
largest_lyapunov=(reordered_lyapunov[:,-1,0])


# plot largest lyapunov exponent
conprob2=conProbability.copy()
conprob2[0]=1e-6
fig, axs = plt.subplots(1, 1, figsize=(10, 10))
axs.plot(conprob2,largest_lyapunov)
axs.set_title('Largest lyapunov exponent')
axs.set_xscale('log')

# average lyapunov exponent
avg_lyapunov=np.mean(reordered_lyapunov[:,-1,:],-1)
fig, axs = plt.subplots(1, 1, figsize=(10, 10))
axs.plot(conprob2,avg_lyapunov)
axs.set_title('Mean lyapunov exponent')
axs.set_xscale('log')


#%% get lyapunov exponent of the entire activities-> use stateful +flatten matrices to pass on+ one output

from RNNcustom_2_fix_2_jacobian import RNNCustom2Fix2_jacobian_flat_1out
from QRDCell2 import QRDcell2_flat
def build_model_lyapunov():
    A_mask=build_masks(nUnit,nInh, con_prob,seed=seed1)
    B_mask=build_masks(nUnit,nInh, con_prob,seed=seed1)
    ...
    # Create your custom RNN cell
    rnn_cell = RNNCustom2Fix2_jacobian_flat_1out(
        nUnit,
        output_activation=tf.keras.layers.ReLU(max_value=1000),
        input_activation=tf.keras.layers.LeakyReLU(alpha=ReLUalpha),
        use_bias=False,
    ...
    #  hidden2 = Dense(10, activation='relu')(hidden1)
    QRDCell=QRDcell2_flat(nUnit=nUnit,start=start)
    hidden1= tf.ensure_shape(hidden1, [batch_size, None, 4 * nUnit**2])
    Rlayer=RNN(QRDCell,stateful=True,return_sequences=True)
    Rmat=Rlayer(hidden1)
    #output_A = Dense(2, activation='tanh',kernel_initializer=GlorotUniform(seed=seed1), kernel_constraint=IEWeightOut(nInh=nInh))(hidden1[0])
    #output_B = Dense(2, activation='tanh',kernel_initializer=GlorotUniform(seed=seed1), kernel_constraint=IEWeightOut(nInh=nInh))(hidden1[1])
    output=Rmat
...
sample_size=2
batch_size=6
x, y, In_ons=makeInOut(batch_size,3,inputdur,nInput,min_dur,max_dur,dt)
...
#%% lyapunov: jacobian and qrd cell combined
from RNNcustom_2_fix_2_jacobian import RNNCustomQRCell
from QRDCell2 import QRDcell2
def build_model_lyapunov():
    A_mask=build_masks(nUnit,nInh, con_prob,seed=seed1)
    B_mask=build_masks(nUnit,nInh, con_prob,seed=seed1)
    ...
    dense_layer = Dense(2, activation='tanh',kernel_initializer=GlorotUniform(seed=seed1), kernel_constraint=IEWeightOut(nInh=nInh))
    # Apply the Dense layer to each LSTM output separately
    output_A = dense_layer(hidden1[0])
    output_B = dense_layer(hidden1[1])
    #output_A = Dense(2, activation='tanh',kernel_initializer=GlorotUniform(seed=seed1), kernel_constraint=IEWeightOut(nInh=nInh))(hidden1[0])
    #output_B = Dense(2, activation='tanh',kernel_initializer=GlorotUniform(seed=seed1), kernel_constraint=IEWeightOut(nInh=nInh))(hidden1[1])
    output=[Concatenate(axis=2)([output_A,output_B]),hidden1[2]]
    model = Model(inputs=[visible, initial_state_A, initial_state_B], outputs=output)
    return model
...
#model_lya.set_weights(model.get_weights())
predictions =model_lya.predict(x)
