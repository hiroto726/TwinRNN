...
from RNNcustom_2_fix2_nRNN import RNNCustom2Fix2_n, RNNCustom2Fix2_n_getstate
...
conProb=[0.005,0.003,0.05]
weight_max=[0.2,0.2,0.2]

#conProb=[0.5]
#weight_max=[0.2]

max_iteration=50000
# specify number of RNNs to train
RNN_num=5
RNN_num=int(RNN_num)
...
def makeInOut(sample_size,trial_num, RNN_num, inputdur,nInput,min_dur,max_dur,dt):
...
def avgAct(activities,In_ons,min_dur,max_dur):
    ...
    dur0_dur=dura[ind1]
    kk=0
    act_avg=np.zeros((min_dur+max_dur,np.shape(activities)[-1]))
    for i in np.arange(np.shape(activities)[0]):
        In_time=In_ons[i,1+((ind1+i+1) % 2):-2:2]
        for j in In_time:
...
def build_model():
    ...
    hidden1=RNN(RNNCustom2Fix2_n(units=nUnit,
                                 RNN_num=RNN_num,
...
def build_model_internal():
    ...
    #hidden1 = SimpleRNN(nUnit,activation='tanh', use_bias=False, batch_size=batch_sz, stateful=False, input_shape=(None, 1), return_sequences=True)(vis_noise)
    # the code below incorporated options to train input kernel within RNN layer
    rnn_output, internal_output =RNN(RNNCustom2Fix2_n_getstate(units=nUnit,
                                 RNN_num=RNN_num,
                                 output_activation=tf.keras.layers.ReLU(max_value=1000),
                                 input_activation=tf.keras.layers.LeakyReLU(alpha=ReLUalpha),
                                 use_bias=False,
...
for weight_ind in range(len(conProb)):
    save=True
    if save:
        ...
        directory = f"{RNN_num}RNNs_prob{conprobability}_weightmax{weight_name}"      
    ...
    x, y, In_ons=makeInOut(sample_size,trial_num,RNN_num,inputdur,nInput,min_dur,max_dur,dt)
    ...
    while (minLoss>Loss_threashold or i % 100!=1) and i<max_iteration and indic==0 :
        x, y, In_ons=makeInOut(2,trial_num,RNN_num, inputdur,nInput,min_dur,max_dur,dt)
        ...
        if i % 100==0:
            ...
            # Plotting data for each subplot
            fig, axs = plt.subplots(RNN_num, 1, figsize=(14, 12))
            
            # Plotting data for each subplot
            for k in range(RNN_num):
                axs[k].plot(y[0, :, 2*k], color='blue')
                axs[k].plot(predictions[0, :, 2*k], color='green')
                axs[k].plot(y[0, :, 2*k+1], color='red')
                axs[k].plot(predictions[0, :, 2*k+1], color='orange') 
                axs[k].set_ylabel(f'RNN {k+1}')
                #axs[k].set_title(f'Plot {i+1}')
            # Adjusting layout
            plt.tight_layout()
            plt.show()
    ...
    fig, axs = plt.subplots(RNN_num, 1, figsize=(7, 2*RNN_num),sharex=True)
    Line=[None]*4
    ind1=In_ons[0,3]-100
    ind2=In_ons[0,5]+100
    ...
    for i in range(RNN_num):
        axs[i].plot(x,y[0,ind1:ind2,2*i],color='black',label='_nolegend_')
        axs[i].plot(x,y[0,ind1:ind2,2*i+1],color='black',label='_nolegend_')
        axs[i].plot(x,predictions[0,ind1:ind2,2*i],color='green',label='Output 0',)
        axs[i].plot(x,predictions[0,ind1:ind2,2*i+1],color='red',label='Output 1')
        
        axs[i].set_ylabel(f'RNN {i+1}')
        ...
        if i==RNN_num-1:
            ...
            axs[i].set_xticks((0,600,1200,1800))
            axs[i].set_xticklabels((0,6,12,18))
        else:
   ...
   # activity_model = Model(inputs=model.input, outputs=outputs)
    ...
    rnn_output, internal_output = activity_model.predict(x)
    _,internal_output_test=activity_model.predict(x)
    ...
    max_range=[min_dur,max_dur+min_dur]
    act_avg_all=[]
    # get average activity
    fig, axs = plt.subplots(1,RNN_num,figsize=(3*RNN_num,7),sharey=True)
    for i in range(RNN_num):
        activities=RNNact[i]
        act_avg=avgAct(activities,In_ons,min_dur,max_dur)
        act_avg_all.append(act_avg)
        
        activities_test=RNNact_test[i]
        act_avg_test=avgAct(activities_test,In_ons,min_dur,max_dur)
        
        # time range to choose max firing time
        # visualize activity of units
        act_avg_test=act_avg_test/(np.max(act_avg_test[max_range[0]:max_range[1], :],axis=0)+1e-10)
        # Reorder the activities based on sorted units
        maxtime_A=np.argmax(act_avg[max_range[0]:max_range[1], :],axis=0)
        sort_filtered_units_A=np.argsort(maxtime_A)
        Cellact= act_avg_test[:, sort_filtered_units_A]
        
        axs[i].imshow(Cellact.T, aspect='auto', cmap=parula,interpolation='none', vmin=0, vmax=1)
        axs[i].set_title(f'RNN{i+1}')
        if i>0:
    ...
    RNN_allweights=model.layers[1].get_weights()
    shapes = [element.shape for element in model.layers[1].get_weights()]
    Rec_w=model.layers[1].get_weights()[1]# recurrent weights
    
 #   in_A,in_B=np.split(RNN_input_kernel,2, axis=1)
 #   Wr_A, Wr_B, S_A, S_B=np.split(RNN_layer_Recurrent_kernel,4, axis=1)
    
    from scipy.io import savemat
    mdict={"RNN_allweights": RNN_allweights,
          "input":x,
          "output":y,
          "In_ons":In_ons,
          "tau":tau,
    ...
    if save:
        os.chdir(savepath)
        savemat(f"model_{minInd}_weights_matlab.m",mdict)
    ...
    ie=[]
    ii=[]
    fig, axs = plt.subplots(2,RNN_num,sharex=True, sharey='row',figsize=(1.8*RNN_num,2)) 
    # Sort units based on the time of maximum activity
    for i in range(RNN_num):
        max_time_ex_A = np.argmax(act_avg_all[i][max_range[0]:max_range[1], :-nInh], axis=0)
        max_time_in_A = np.argmax(act_avg_all[i][max_range[0]:max_range[1], -nInh:], axis=0)
        ...
        # sort recurrent diagonal block
        Wr_sort_A=Rec_w[nUnit*i:nUnit*(i+1),nUnit*i:nUnit*(i+1)]
        ...
        axs[0,i].plot(np.delete(order_A,Bin),np.delete(exexavg_A,Bin),label='Ex→Ex')
        axs[0,i].plot(order_A,exinavg_A,label='Ex→In')
        axs[0,i].title.set_text(f'RNN {i+1}')
        axs[0,i].spines['right'].set_visible(False)
        axs[0,i].spines['top'].set_visible(False)
        #axs[0,0].legend(loc='lower center')
        axs[1,i].plot(np.delete(order_A,Bin),np.delete(ininavg_A,Bin),label='In→In')
        axs[1,i].plot(order_A,inexavg_A,label='In→Ex')
        axs[1,i].spines['right'].set_visible(False)
        axs[1,i].spines['top'].set_visible(False)
        if i>0:
            axs[0,i].spines['left'].set_visible(False)
            axs[0,i].tick_params(left=False, labelleft=False)  # Remove left y-ticks and labels
            axs[1,i].spines['left'].set_visible(False)
            axs[1,i].tick_params(left=False, labelleft=False)  # Remove left y-ticks and labels   
        if i==RNN_num-1:
            axs[0,i].legend(bbox_to_anchor=(2, 1), loc='upper right', borderaxespad=0.)
            axs[1,i].legend(bbox_to_anchor=(2, 1), loc='upper right', borderaxespad=0.)
    if save:
