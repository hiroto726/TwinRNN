...
def makeInOut(sample_size,trial_num,inputdur,nInput,min_dur,max_dur,dt):
    ...
    for i in range(sample_size):
        ...
        for j in np.arange(trial_num):
            ...
            y[i,in_start:vecbf,0]=np.linspace(0,1,num=Dur)-0.5        # relative timing 1
            #y[i,vec:vecbf,1]=np.arange(-0.5,-0.5+(Dur/max_dur)-1e-10,1/max_dur)
            y[i,in_start:vecbf,1]=np.linspace(0,1,num=Dur)-0.5 
...
for weight_ind in range(len(weight_max)):
    ...
    if save:
        ...
        directory = f"2sameInputs__latterhalf_tau100_prob{conprobability}_weightmax{weight_name}_fix3"      
