...
def makeInOutTest(int_list,inputdur,nInput,min_dur,max_dur,dt):
    ...
    for i in range(sample_size):
        ...
        for j in np.arange(trial_num):
            ...
            y[i,vec:vecbf,1]=np.arange(-0.5,-0.5+(Dur/max_dur)-1e-10,1/max_dur)
            y[i,vec:vecbf,2]=np.where(Dur >= 0.5*(min_dur+max_dur), -0.2, 0.2)     # context timing 2
            y[i,vec:vecbf,3]=np.where(Dur >= 0.5*(min_dur+max_dur), -0.2, 0.2)

            if Dur <= 0.5*(min_dur+max_dur):
...
for weight_ind in range(len(weight_max)):
    ...
    if save:
        ...
        directory = f"2sameInputs__diffoutputs_prob{conprobability}_weightmax{weight_name}_fix3"      
...
# Show the plot
if save:
    plt.savefig(os.path.join(savepath,f"Sequentiality_Index"),transparent=True,dpi=600)
plt.show
