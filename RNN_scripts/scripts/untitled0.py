...
from SaturatingRelu import CustomSaturatingReLU, custom_log_saturate_relu_fn
...
def makeInOut(sample_size,trial_num,inputdur,nInput,min_dur,max_dur,dt,brown_scale, rank, exp):
    ...
    brown_noise=brown_scale*low_rank_brown_noise_2(total_time_orig, nUnit, rank, sample_size, exp)
    x=np.concatenate((x,brown_noise),axis=2)
...
from RNNcustom_2_fix_2_full_brown import RNNCustom2Fix2full_brown
def build_model():
    ...
    visible = Input(batch_shape=(batch_size, None, nInput+nUnit))
    ...
    rnn_cell = RNNCustom2Fix2full_brown(
        nUnit,
        output_activation=lambda x: custom_log_saturate_relu_fn(x, alpha=0.0, max_value=200.0), 
        input_activation=lambda x: custom_log_saturate_relu_fn(x, alpha=ReLUalpha, max_value=200.0), 
        use_bias=False,
...
for weight_ind in tind:
    ...
    axes[0].imshow(brown_cat[0], cmap="gray", origin="upper", aspect="equal")
    axes[0].set_title("Aspect Ratio: Equal")
    # Second subplot with aspect ratio 'auto'
    im = axes[1].imshow(brown_cat[0], cmap="gray", origin="upper", aspect="auto")
    ...
    while (minLoss>Loss_threashold or i % 100!=1) and i<max_iteration+1 and indic==0 :
        x, y, In_ons, Wbin=makeInOutstateful(batch_size,12,inputdur,nInput,min_dur,max_dur,dt,batch_num,brown_scale, rank, exp)
        ...
        if i % evalloop==0:
            ...
            x, y, In_ons=makeInOut(batch_size,trial_num,inputdur,nInput,min_dur,max_dur,dt,brown_scale, rank, exp)
