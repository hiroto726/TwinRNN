...
conProb=[0.0003,0.003,0.00001,0.00003,0.03]
...
for weight_ind in range(len(conProb)):
   ...
   # activity_model = Model(inputs=model.input, outputs=outputs)
    ...
    plt.figure()
    plt.imshow(internal_output[0,:,nUnit:2*nUnit],aspect='auto',interpolation='none')
    plt.colorbar()   
    
    weight_all=model.get_weights()
    plt.figure()
    plt.imshow(weight_all[1],aspect='auto',interpolation='none')
    plt.colorbar()    
