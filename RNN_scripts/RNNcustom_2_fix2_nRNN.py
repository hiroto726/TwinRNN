...
class RNNCustom2Fix2_n(AbstractRNNCell):
...
#        self.output_size = self.units
    ...
    def build(self, input_shape):
        self.kernel = self.add_weight(
            shape=(input_shape[-1], self.units*self.RNN_num),
            name="kernel",
    ...
    def call(self, inputs, states, training=False):
        ...
        h = backend.dot(inputs, self.kernel)# get input kernel
...
#@tf.keras.saving.register_keras_serializable(package="RNNCustom")
class RNNCustom2Fix2_n_getstate(AbstractRNNCell):
