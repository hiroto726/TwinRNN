...
class RNNCustom2Fix2full(AbstractRNNCell):
...
# make model with only one rnn
class RNNCustom1(AbstractRNNCell):
...
#        self.output_size = self.units
    ...
    @property
    def output_size(self):
        return self.units  # Must be an integer
    ...
    def call(self, inputs, states, training=False):
        po_A_raw = states[0]  # Unpack the states
        ...
        output_A = h + backend.dot(po_A, Wr_A) + tf.random.normal(
            [batch_size, self.units], mean=0, stddev=self.noisesd, seed=self.seed)
