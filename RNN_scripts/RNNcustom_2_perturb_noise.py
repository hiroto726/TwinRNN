...
class RNNCustom2FixPerturb_noise(AbstractRNNCell):
...
#        self.output_size = self.units
    ...
    def call(self, inputs, states, training=False):
        ...
        if self.pert_state==0:
            ...
            output_A=tf.where(tf.math.equal(iteration,perturb_ind[:,0:1]),output_A_noise,output_A) # add noise only if iteration=pertrub_ind
        else:
            ...
            output_B=tf.where(tf.math.equal(iteration,perturb_ind[:,0:1]),output_B_noise,output_B)            
