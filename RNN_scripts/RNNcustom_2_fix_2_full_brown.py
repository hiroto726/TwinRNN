...
#@tf.keras.saving.register_keras_serializable(package="RNNCustom")
class RNNCustom2Fix2full_brown_indep(AbstractRNNCell):
# made it so that brown noise is low rank and it has fixed sets of random vectors.
    ...
    def __init__(
        ...
        tau=10,
        noisesd=0,
        noise_weights_A=0,# it should be a tensorflow vector of shape (rank, units)
        noise_weights_B=0,# it should be a tensorflow vector of shape (rank, units)
        **kwargs,
    ):
        if units <= 0:
            raise ValueError(
...
#        self.seed_generator = backend.random.SeedGenerator(seed)
        ...
        self.kernel_trainable=kernel_trainable
        self.seed=seed
        self.noise_weights_A=tf.cast(noise_weights_A, dtype=tf.float32) # it should be a tensorflow vector of shape (rank, units)
        self.noise_weights_B=tf.cast(noise_weights_B, dtype=tf.float32) # it should be a tensorflow vector of shape (rank, units)
...
#        self.output_size = self.units
    ...
    def call(self, inputs, states, training=False):
        ...
        inputs_0=inputs[:,0:1]# batch_size * input_dim(1)
        inputs_brown=inputs[:,1:]# batch_size * 2*rank
        brownA, brownB = tf.split(inputs_brown, num_or_size_splits=2, axis=-1)
        h = backend.dot(inputs_0, self.kernel)
        h_A, h_B = tf.split(h, num_or_size_splits=2, axis=1)
        ...
        # add brownian noise
        brown_noise_A = backend.dot(brownA,self.noise_weights_A)#(batch,rank),(rank,units)
        brown_noise_B = backend.dot(brownB,self.noise_weights_B)#(batch,rank),(rank,units)
