...
class RNNCustom2sameweights_JacobianSynch(AbstractRNNCell):
...
#        self.output_size = self.units
    ...
    def call(self, inputs, states, training=False):
        ...
        # transpose matrices for calculating jacobian
        Wt=tf.transpose(Wr_A,perm=[1,0])
        St=tf.transpose(S_A,perm=[1,0])
        xAt=po_A_raw
        with tf.GradientTape() as tape:
            tape.watch(xAt)   
            if self.output_activation is not None:
                reluxt = self.output_activation(xAt) 
        act_jacobian=tape.batch_jacobian(reluxt, xAt) #(batch, unit, unit)
        ...
        if self.input_activation is not None:
            ...
            output_A=tf.where(output_A>0,output_A,self.alpha*output_A)
            output_B=tf.where(output_B>0,output_B,self.alpha*output_B)
        
        output_A=po_A_raw+(output_A-po_A_raw)/self.tau
        output_B=po_B_raw+(output_B-po_B_raw)/self.tau
        
        
        reluxt_expanded = tf.expand_dims(reluxt, axis=-1)# Shape (batch, unit, 1)
        var1=tf.matmul(Wt+St,reluxt_expanded)# Shape (batch, unit, 1)
        var1 = tf.squeeze(var1, axis=-1)# Shape (batch, unit)
        with tf.GradientTape() as tape:
            tape.watch(var1)
            if self.input_activation is not None:
                #output_A = self.input_activation(output_A)# there is bug in tensorflow that prevents calculation of jacobian for leaky relu
                #output_B = self.input_activation(output_B)# there is bug in tensorflow that prevents calculation of jacobian for leaky relu
                var2=tf.where(var1>0,var1,self.alpha*var1)
        g_jacobian=tape.batch_jacobian(var2, var1)#(batch, unit, unit)
        
        batch_size = tf.shape(inputs)[0]
        #get entire jacobian of offsets
        eyes=(1-(1/self.tau))*tf.eye(self.units,self.units,batch_shape=[batch_size])#(batch, unit, unit)
        jacobian=(1/self.tau)*tf.matmul(g_jacobian,tf.matmul(Wt-St,act_jacobian))
        jacobian+=eyes
