...
from RNNcustom_3 import RNNCustom3
from CustomConstraint2 import IEWeight, IEWeightOut
from WInitial_2 import OrthoCustom2
...
if save:
    ...
    directory = r"2sameInputs_reluMax_6s12s_fewTrials_RNN2ver3"      
...
trial_num=2
con_prob=0.1 # probability of connection between neurons
...
def build_model():
    ...
    output=RNN(RNNCustom3(nUnit, 
                          ...
                          recurrent_initializer=OrthoCustom2(gain=0.5, seed=seed1 , nUnit=nUnit, nInh=nInh, W_A=A_mask, W_B=B_mask,conProb=con_prob),
                          dense_initializer=GlorotUniform(seed=seed1),
                          recurrent_constraint=IEWeight(nInh=nInh,A_mask=A_mask,B_mask=B_mask),
                          dense_constraint=IEWeightOut(nInh=nInh),
...
for ss in range(2):
    ...
    axs[ss].plot(predictions[ss, :, 0], color='green',label='Prediction')
    axs[ss].plot(y[ss, :, 1], color='red')
    axs[ss].plot(predictions[ss, :, 1], color='orange')      
...
for i in range(4):
    ...
    predictions = model.predict(x)
    axs[i].plot(y[0,:,0],color='blue',label='Target 0')
    axs[i].plot(predictions[0,:,0],color='green',label='Prediction 0')
    axs[i].plot(y[0,:,1],color='red',label='Target 1')
    axs[i].plot(predictions[0,:,1],color='orange',label='Prediction 1')
plt.tight_layout()
if save:
    plt.savefig(os.path.join(savepath,f"Output_4_types"),transparent=True,dpi=200)
plt.show()
...
output1=output_and_activities[0] # output of the rnn model
...
maxact=np.max(input_activities[max_range[0]:max_range[1], :], axis=0)
max_Unit_sort=np.argsort(-maxact)# this will sort the maximum activity in descending order
good_units=max_Unit_sort[:int(len(max_Unit_sort)/2)]
sorted_units = np.argsort(max_times)
sort_filtered_units=[x for x in sorted_units if x in good_units]

Cellact=input_activities[:,sorted_units]


# Find the time of maximum activity for each unit    
input_activities_2=input_activities/(np.max(input_activities[max_range[0]:max_range[1], :],axis=0)+1e-10)
# Reorder the activities based on sorted units
sorted_activities = input_activities_2[:, sort_filtered_units]
plt.imshow(sorted_activities.T, aspect='auto', cmap='viridis',interpolation='none', vmin=0, vmax=1)
plt.colorbar(label='Activity')


# save figure
if save:
    plt.savefig(os.path.join(savepath,f"Inh_Unit_activity_epoch{k+1}_sortBy_12s"),transparent=True,dpi=600)
...
# %%
# get weights explicitly
#for layer in model.layers: print(layer.get_config(), layer.get_weights())
dense_layer_kernel=model.layers[2].get_weights()[0]
dense_layer_bias=model.layers[2].get_weights()[1]
RNN_layer_Recurrent_kernel=model.layers[1].get_weights()[1]
RNN_layer_kernel=model.layers[1].get_weights()[0]
eigval=np.linalg.eig(RNN_layer_Recurrent_kernel)
plt.plot(np.abs(eigval[0]))




# analyze weights distribution
input_activities= act_avg
max_range=[min_dur,min_dur+max_dur]

# for filetering excitory units
max_times = np.argmax(input_activities[max_range[0]:max_range[1], :-nInh], axis=0)
# Sort units based on the time of maximum activity
maxact=np.max(input_activities[max_range[0]:max_range[1], :-nInh], axis=0)
max_Unit_sort=np.argsort(-maxact)# this will sort the maximum activity in descending order
good_units=max_Unit_sort[:int(len(max_Unit_sort)/2)]
sorted_units = np.argsort(max_times)
sort_filtered_units_ex=[x for x in sorted_units if x in good_units]
sort_units_time_ex=max_times[sort_filtered_units_ex]
Ex_num=np.shape(sort_units_time_ex)[0]

# for filtering inhibitory units
max_times = np.argmax(input_activities[max_range[0]:max_range[1], -nInh:], axis=0)
# Sort units based on the time of maximum activity
maxact=np.max(input_activities[max_range[0]:max_range[1], -nInh:], axis=0)
max_Unit_sort=np.argsort(-maxact)# this will sort the maximum activity in descending order
good_units=max_Unit_sort[:int(len(max_Unit_sort)/2)]
sorted_units = np.argsort(max_times)
sort_filtered_units_in=[x for x in sorted_units if x in good_units]
sort_units_time_in=max_times[sort_filtered_units_in]
In_num=np.shape(sort_filtered_units_in)[0]
sort_filtered_units_in=[x-nInh for x in sorted_units if x in good_units]# add Ex_num 

# modify the weight matrix to only include filtered units
filtInd=np.concatenate((sort_filtered_units_ex,sort_filtered_units_in),axis=0)
weight_mat=RNN_layer_Recurrent_kernel[filtInd,:][:,filtInd]
plt.imshow(np.log(np.abs(weight_mat)),aspect='auto', cmap='viridis',interpolation='none',vmin=-9, vmax=0)

# get weights from prior and later units
Bin=40 #get +-Bin units
exex=np.empty((2*Bin+1,Ex_num))
exex[:]=np.nan
inex=exex.copy()
inin=np.empty((2*Bin+1,In_num))
inin[:]=np.nan
exin=inin.copy()

# get the relationship between max firing rate for in and ex
sort_units_time_ex=sort_units_time_ex.reshape((Ex_num,1))
sort_units_time_in=sort_units_time_in.reshape((In_num,1))
...
for i in range(len(sort_filtered_units_ex)):
    exex[max(0,Bin-i):min(Bin+Ex_num-i,2*Bin+1),i]=weight_mat[max(0,i-Bin):min(Ex_num,i+Bin+1),i]
    inex[max(0,Bin-Ex_ind[i]):min(Bin+In_num-Ex_ind[i],2*Bin+1),i]=weight_mat[Ex_num+max(0,Ex_ind[i]-Bin):Ex_num+min(In_num,Ex_ind[i]+Bin+1),i]
    

for i in range(len(sort_filtered_units_in)):
    inin[max(0,Bin-i):min(Bin+In_num-i,2*Bin+1),i]=weight_mat[Ex_num+max(0,i-Bin):Ex_num+min(In_num,i+Bin+1),Ex_num+i]
    exin[max(0,Bin-In_ind[i]):min(Bin+Ex_num-In_ind[i],2*Bin+1),i]=weight_mat[max(0,In_ind[i]-Bin):min(Ex_num,In_ind[i]+Bin+1),Ex_num+i]
...
plt.figure()
plt.plot(np.delete(order,Bin),np.delete(exexavg,Bin),label='Ex Ex')
plt.plot(order,exinavg,label='Ex In')
plt.plot(np.delete(order,Bin),np.delete(ininavg,Bin),label='In In')
plt.plot(order,inexavg,label='In Ex')
plt.legend(loc='lower right')
if save:
    plt.savefig(os.path.join(savepath,f"weight_distribution_sort_by_6s"),transparent=True,dpi=600)
